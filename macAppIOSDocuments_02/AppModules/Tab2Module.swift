                ...
                HStack(spacing: 12) {
                    TextField("😊", text: $custom)
                        ...
                        .onChange(of: custom) { val in
                            if val.unicodeScalars.first != nil, val.count > 1 {
...
struct Tab2View: View {
    ...
    @State private var cachedApiKey: String = KeychainHelper.load(forKey: "gptApiKey") ?? ""
    ...
    var body: some View {
        NavigationView {
            ...
            .onAppear {
                cachedApiKey = KeychainHelper.load(forKey: "gptApiKey") ?? ""
            }
            .toolbar {
    ...
    static func buildUserPrompt(_ pdfText: String) -> (prompt: String, hints: ExtractedHints) {
        ...
        // "Te betalen € X per Bank" is the actual bank transfer instruction — 100% reliable
        // "Netto te betalen" can be confused with other numbers by PDFKit column ordering
        let nettoPatterns = [
            #"Te betalen\s*€\s*[\d]+[.,][\d]+\s*per Bank"#,          // Werckpost: exact bank instruction
            #"Net\s+pay\s+[\d]+[.,][\d]+\s+[\d]+[.,][\d]+"#,         // English: "Net pay 494,74 494,74"
            #"[Nn]etto\s+te\s+betalen\s+[\d]+[.,][\d]+\s*={3,}"#,    // Dutch: followed by === (end of table)
            #"Netto te betalen\s+[\d]+[.,][\d]+"#,                    // Dutch fallback
            #"Te betalen\s*€?\s*[\d]+[.,][\d]+\s*per Bank"#,          // Dutch variant without €
        ...
        let taxPatterns = [
            // Explicit named labels — most reliable
            #"[Ll]oonheffingen?\s+-[\d]+[.,][\d]+"#,
            #"[Ww]age\s+tax\s+-[\d]+[.,][\d]+"#,
            #"[Ii]ncome\s+tax\s+-[\d]+[.,][\d]+"#,
            #"[Ll]ohnsteuer\s+-[\d]+[.,][\d]+"#,
            #"[Pp]ayroll\s+tax\s+-[\d]+[.,][\d]+"#,
            // Werckpost/NBBU: "T <grondslag> -<tax> <grondslag> -<tax>" — tight pattern, no wildcards
            #"\bT\b\s+[\d]+[.,][\d]+\s+-([\d]{2,}[.,][\d]+)\b"#,
        ]
        var taxFound = false
        for p in taxPatterns {
            var searchStart = pdfText.startIndex
            var bestTax: Double = 0
            // Collect all matches, take the largest (ignore tiny PAWW/AZV amounts)
            while let m = pdfText.range(of: p, options: .regularExpression, range: searchStart..<pdfText.endIndex) {
                let row = String(pdfText[m])
                let negs = row.matches(of: #/-([\d]+[.,][\d]+)/#)
                    .compactMap { Double(String(row[$0.range]).replacingOccurrences(of: "-", with: "").replacingOccurrences(of: ",", with: ".")) }
                    .filter { $0 > 5.0 } // skip penny deductions (PAWW 0.16 etc)
                if let v = negs.max(), v > bestTax { bestTax = v }
                searchStart = m.upperBound
            }
            if bestTax > 0 {
                let taxStr = String(format: "%.2f", bestTax).replacingOccurrences(of: ".", with: ",")
    ...
    private func extractDatesWithGPT() {
        cachedApiKey = KeychainHelper.load(forKey: "gptApiKey") ?? ""
        guard !cachedApiKey.isEmpty else { return }
        ...
        Task {
            ...
            for doc in docs {
                ...
                // Log key lines from PDF text for debugging (netto/bruto area)
                let debugLines = pdfText
                    .components(separatedBy: .newlines)
                    .filter { line in
                        let l = line.lowercased()
                        return l.contains("netto") || l.contains("bruto") || l.contains("betalen") || l.contains("salary") || l.contains("net pay")
                    }
                    .prefix(10)
                    .joined(separator: " | ")
                // Include text in message so logger captures it
                info("PDF[\(doc.name)] netto/bruto lines: \(debugLines.isEmpty ? "NONE FOUND" : debugLines)", "pdfText.keyLines", [:])
                ...
                req.httpMethod = "POST"
                req.setValue("Bearer \(cachedApiKey)", forHTTPHeaderField: "Authorization")
                ...
                guard let result = gptResult else {
                ...
                // Validate GPT netto: if it equals bruto (±0.50€), GPT confused the fields
                // This happens when PDFKit renders a payslip where netto < bruto (big deductions)
                let gptNettoClean: Double = {
                    guard gptNetto > 0 else { return 0 }
                    if bruto > 0, abs(gptNetto - bruto) < 0.50 { return 0 } // GPT returned bruto as netto
                    return gptNetto
                }()

                let netto: Double = {
                    if let hint = extractedHints.netto { return hint }
                    if gptNettoClean > 0 { return gptNettoClean }
                    // Fallback 1: nettoLoon hint (netto ≈ nettoLoon when no vergoedingen)
                    if let hintNL = extractedHints.nettoLoon, hintNL > 0 { return hintNL }
                    // Fallback 2: compute bruto - GPT deductions
                    // Catches cases where PDFKit text doesn't match any regex pattern
                    let gptDeductions = num("tax") + num("pension") + num("azv") +
                                        num("paww") + num("wga") + num("zvw") + num("wia")
                    let gptVergoedInline = num("vergoedingen")
                    if gptDeductions > 0 {
                        let computed = bruto - gptDeductions + gptVergoedInline
                        if computed > 0 && computed < bruto * 1.5 {
                            info("netto computed from deductions", "bruto-deductions", ["bruto": bruto, "deductions": gptDeductions, "computed": computed])
                            return computed
                        }
                    }
                    return bruto
                ...
                info("RESULT[\(doc.name)] hint.netto=\(extractedHints.netto.map{String($0)} ?? "nil") gptNetto=\(num("netto")) bruto=\(bruto) → netto=\(netto)", "computedValues", [:])

                // nettoLoon: prefer hint; fall back to GPT only if it's a valid intermediate value
                let nettoLoon: Double = {
                    if let hintNL = extractedHints.nettoLoon, hintNL > 0, hintNL <= netto + 0.01 {
                        return hintNL
                    }
                    guard gptNettoLoon > 0, gptNettoLoon <= netto + 0.01, gptNettoLoon != bruto else { return 0 }
                    return gptNettoLoon
                }()
                ...
                let entry = SalaryAnalysis(
                    ...
                    needsReview: bruto == 0 || (netto > (bruto + vergoedingen) * 1.5 + 1)
                )
