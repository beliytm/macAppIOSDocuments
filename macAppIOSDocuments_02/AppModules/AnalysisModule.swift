...
final class AnalysisStorage: ObservableObject {
    ...
    /// Merge entries with same company + same period into one combined entry
    func merged(_ source: [SalaryAnalysis]) -> [SalaryAnalysis] {
        // Group by: company name + dateFrom (day) + dateTo (day)
        let calendar = Calendar.current
        var groups: [String: [SalaryAnalysis]] = [:]
        for e in source {
            let from = calendar.startOfDay(for: e.dateFrom)
            let to   = calendar.startOfDay(for: e.dateTo)
            let key  = "\(e.documentName)|\(from.timeIntervalSince1970)|\(to.timeIntervalSince1970)"
            groups[key, default: []].append(e)
        }

        return groups.values.map { group -> SalaryAnalysis in
            ...
            return SalaryAnalysis(
                ...
                dateTo: base.dateTo,
                bruto:               group.reduce(0) { $0 + $1.bruto },
                netto:               group.reduce(0) { $0 + $1.netto },
                normalHours:         group.reduce(0) { $0 + $1.normalHours },
                irregularHours:      group.reduce(0) { $0 + $1.irregularHours },
                tax:                 group.reduce(0) { $0 + $1.tax },
                pension:             group.reduce(0) { $0 + $1.pension },
                azv:                 group.reduce(0) { $0 + $1.azv },
                paww:                group.reduce(0) { $0 + $1.paww },
                vacationPay:         group.map { $0.vacationPay }.max() ?? 0,
                nettoLoon:           group.reduce(0) { $0 + $1.nettoLoon },
                arbeidskorting:      group.reduce(0) { $0 + $1.arbeidskorting },
                vakantiegeld:        group.reduce(0) { $0 + $1.vakantiegeld },
                ww:                  group.reduce(0) { $0 + $1.ww },
                wia:                 group.reduce(0) { $0 + $1.wia },
                vakantiedagen:       group.map { $0.vakantiedagen }.max() ?? 0,
                toeslagen:           group.reduce(0) { $0 + $1.toeslagen },
                vergoedingen:        group.reduce(0) { $0 + $1.vergoedingen },
                zvw:                 group.reduce(0) { $0 + $1.zvw },
                wga:                 group.reduce(0) { $0 + $1.wga },
                loonLBPH:            group.map { $0.loonLBPH }.max() ?? 0,
                loonSV:              group.map { $0.loonSV }.max() ?? 0,
                opbouwVakantiedagenUren: group.reduce(0) { $0 + $1.opbouwVakantiedagenUren },
                loonOnregelmatig:    group.reduce(0) { $0 + $1.loonOnregelmatig },
                loonOverwerk:        group.reduce(0) { $0 + $1.loonOverwerk },
                needsReview:         group.contains { $0.needsReview }
            )
...
struct AnalysisView: View {
    ...
    private var activeEntries: [SalaryAnalysis] {
        let raw = selectedTab == 0 ? store.lastBatchEntries : store.entries
        return store.merged(raw)
    }
